    ...
    for (p_name, p) in &variant.properties {
        ...
        if let Some(obj) = &schema_obj.object {
            ...
            let TypeData::Struct { fields } = ty else {
                anyhow::bail!("Expected obj to be a struct");
            };

            return Ok((p_name.to_string(), EnumVariantType::Struct { fields }));
        }
    ...
    for (p_name, p) in &obj.properties {
            ...
            && enum_vals.len() == 1
        {
            match &enum_vals[0].as_str() {
                Some(v) => {
                    discriminator_field_name = Some(p_name.to_string());
                    discriminator = Some(v.to_string());
                }
